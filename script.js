...
function loadMovies() {
    ...
    usersRef.on('value', userSnapshot => {
        ...
        moviesRef.on('value', movieSnapshot => {
            ...
            // 🌟 NEW: Randomly display the Random Discovery Section
            if (Math.random() < 0.5) { 
                fetchAndRenderRandomDiscoveries();
            } else {
                document.getElementById('randomDiscoveryContainer').innerHTML = '';
                document.getElementById('randomDiscoveryHeader').style.display = 'none';
            }
              ...
              <div class="movie-meta">
                  ...
                  <div class="like-section">
                      ...
                      <i 
                        ...
                        onclick="event.stopPropagation(); shareFirebaseMovie('${movie.name}')"
                      ></i>
                  </div>
              </div>
...
// 📤 NEW: Universal Share Helper
function shareContent(text, url) {
    if (navigator.share) {
        navigator.share({
            title: "Movie Streak Discovery",
            text: text,
            url: url
...
// 📤 NEW: Share functions for Firebase and Random movies
function shareFirebaseMovie(movieName) {
    const title = `Check out this great movie: ${movieName} on Movie Streak!`;
    const url = window.location.href; // Use the app's main URL
    shareContent(title, url);
}

function shareRandomMovie(title, url) {
    const text = `Check out this random movie: ${title}`;
    shareContent(text, url);
}

function shareMovieOfTheDay() {
    if (!movieOfTheDay) return;
    const title = `Check out the Movie of the Day: ${movieOfTheDay.name}`;
...
function displayMovieOfTheDay() {
    ...
    if (movieOfTheDay) {
        ...
        // 🔄 NEW: Populate the user profile section
        const motdUsersContainer = document.getElementById('motdUsers');
        motdUsersContainer.innerHTML = '';
        
        const currentMovieInList = allMovies.find(m => m.key === movieOfTheDay.key);
        
        const interactingUserIds = new Set();
        if (currentMovieInList) {
            if (currentMovieInList.watchedBy) Object.keys(currentMovieInList.watchedBy).forEach(id => interactingUserIds.add(id));
            if (currentMovieInList.wannaWatchBy) Object.keys(currentMovieInList.wannaWatchBy).forEach(id => interactingUserIds.add(id));
        }
        
        const displayUsers = Array.from(interactingUserIds).filter(id => id !== currentUserId).slice(0, 5);

        if (displayUsers.length > 0) {
            displayUsers.forEach(userId => {
                const user = movieUsers[userId];
                if (user) {
                    const img = document.createElement('img');
                    img.src = user.picUrl;
                    img.alt = `${user.name} added this`;
                    img.className = 'motd-user-pic';
                    motdUsersContainer.appendChild(img);
                }
            });
        }
        
        // Hide/Show the new 'Add' icon based on whether it's a personalized highlight
        document.getElementById('addMotdIcon').style.display = (document.getElementById('motdSubtitle').textContent.includes('Highlight')) ? 'none' : 'block';
...
// 🚀 START: EXISTING DISCOVERY CARDS FUNCTION
function renderDiscoveryCards() {
    const container = document.getElementById('discoveryCardsContainer');
    const header = document.getElementById('discoveryHeader');
    container.innerHTML = '';

    if (allMovies.length < 5 || !currentUserId) {
        header.style.display = 'none';
        return;
    }
    
    // 1. Filter movies the current user hasn't already interacted with
    let potentialSuggestions = allMovies.filter(movie => {
        const isInteracted = (movie.watchedBy && movie.watchedBy[currentUserId]) ||
                             (movie.wannaWatchBy && movie.wannaWatchBy[currentUserId]) ||
                             (movie.likes && movie.likes[currentUserId]);
        return !isInteracted;
    });
    ...
    // 2. Score/Rank the movies based on community interest
    potentialSuggestions.forEach(movie => {
        // Score = (Total Likes) + (Total Wanna Watch * 1.5)
        const totalLikes = Object.keys(movie.likes || {}).length;
        const totalWannaWatch = Object.keys(movie.wannaWatchBy || {}).length;
        movie.score = totalLikes + (totalWannaWatch * 1.5);
    });

    // 3. Sort by score (most popular) and select the top 6
    const topSuggestions = potentialSuggestions
        .sort((a, b) => b.score - a.score)
        .slice(0, 6); // Display up to 6 cards

    // 4. Render the cards
    topSuggestions.forEach(movie => {
        const isLiked = movie.likes && movie.likes[currentUserId];
        const isWannaWatch = movie.wannaWatchBy && movie.wannaWatchBy[currentUserId];
        ...
        card.innerHTML = `
            ...
            <div class="action-buttons">
                <i 
                    class="fas fa-heart ${isLiked ? 'liked' : ''}"
                    onclick="event.stopPropagation(); toggleLike('${movie.key}')"
                    title="Like"
                ></i>
                <i 
                    class="fas fa-bookmark ${isWannaWatch ? 'wanna-watch' : ''}"
                    onclick="event.stopPropagation(); toggleWannaWatch('${movie.key}')"
                    title="Wanna Watch"
                ></i>
                <i 
        ...
        `;
        // Make the entire card click to show details, just like the movie grid
        card.onclick = () => showMovieDetails(movie.key);
        container.appendChild(card);
    });
    
    header.style.display = 'block'; // Show the header once cards are rendered
}
// 🚀 END: EXISTING DISCOVERY CARDS FUNCTION

// 🌟 NEW: Random Discovery Section Logic
async function fetchAndRenderRandomDiscoveries() {
    const container = document.getElementById('randomDiscoveryContainer');
    const header = document.getElementById('randomDiscoveryHeader');
    container.innerHTML = '';

    // Choose a random page for variation
    const page = Math.floor(Math.random() * 50) + 1;
    const url = `${tmdbBaseUrl}/discover/movie?api_key=${tmdbApiKey}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}`;

    try {
        const response = await fetch(url);
        const data = await response.json();
        
        // Select up to 8 popular movies with posters to fit the 2x4 grid look
        const randomMovies = data.results.filter(m => m.poster_path).slice(0, 8);

        if (randomMovies.length > 0) {
            randomMovies.forEach(movie => {
                const posterUrl = `${tmdbImageBaseUrl}${movie.poster_path}`;
                const detailLink = `https://www.themoviedb.org/movie/${movie.id}`;

                const li = document.createElement('li');
                li.className = 'movie-card random-card'; 
                // Card click opens an external page for "more information"
                li.onclick = () => window.open(detailLink, '_blank'); 
                ...
                li.innerHTML = `
                    ...
                    <div class="movie-card-content">
                        ...
                        <div class="movie-meta">
                            <span class="movie-genre">TMDb Popular</span>
                            <div class="like-section">
                                <i class="fas fa-heart like-icon" title="Like"></i>
                                <i class="fas fa-bookmark wanna-watch-icon" title="Wanna Watch"></i>
                                <i class="fas fa-share-alt share-icon" onclick="event.stopPropagation(); shareRandomMovie('${movie.title}', '${detailLink}')"></i>
                            </div>
                        </div>
                        <span class="movie-timestamp">Click for details</span>
                    </div>
                `;
        ...
        console.error("Error fetching random TMDb movies:", error);
        header.style.display = 'none';
    }
}
// ------------------------------------------
...
document.getElementById("share-button").addEventListener("click", function () {
    const title = "Help me build a movie list on Movie Streak!";
    const url = window.location.href;
    shareContent(title, url);
});
